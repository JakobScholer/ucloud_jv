...
if __name__ == '__main__':
    ...
    if len(argv) == 2:
        ...
        if str(argv[1]) == "ec":                        # show energy curves for stringfiles of reaction
            ...
            energy_curve_all_reactions(folder="blackbox/output/test")
        ...
        # ---------------Calc---------------Calc---------------Calc---------------Calc---------------Calc---------------
            ...
            make_reactions(blackbox=True, string_data=["CC(N)=C(N)C(O)=C(N)C(O)=C(C)O"], max_energy=200, frozen=[3,5,6,7,8,9,11,14], number_of_processes=4, debug=False)
            ...
            for i in range(20):
                copytree("blackbox/output/test/original", f"blackbox/output/test/{i}")
                #make_single_reaction(stringfile=f"blackbox/output/test/{i}/stringfile.xyz", number_of_processes = 1, debug = False)
                run_gsm_round_initial(output_folder = f"blackbox/output/test/{i}", reaction_folder = "", logfile = False)
