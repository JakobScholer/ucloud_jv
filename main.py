...
if __name__ == '__main__':
    if len(argv) == 2:
        ...
        if str(argv[1]) == "ec":                        # show energy curves for stringfiles of reaction
            visualize_energy_curves(folder="blackbox/output/CC=CC=CC=CO/reaction0005")
        elif str(argv[1]) == "ec_all_reactions":        # show energy curves for all reactions
            energy_curve_all_reactions(folder="blackbox/output/CC=C")
            ...
            make_cut_dag(blackbox=False, stringfile="blackbox/output/CC=CC=CC=CC/reaction0070/stringfile.xyz0070",visual_cut_dag=True , visual_stringfiles=True, DEBUG_MODE=False)

        # ---------------Calc---------------Calc---------------Calc---------------Calc---------------Calc---------------
        elif str(argv[1]) == "smiles_to_reactions_bb":  # Compute all reactions as stringfiles from smiles
            make_reactions(blackbox=True, string_data=["CC=C"], max_energy=200, debug=True)
