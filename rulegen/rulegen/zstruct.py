...
def prepare_ssm(isomer_path, i: int):
    #os.removedirs("ssm/scratch")
    os.system("rm -r ssm/scratch")
    os.mkdir("ssm/scratch")
    #zstruct_path = "zstruct/scratch"
    ssm_path = "ssm/scratch"
    ID = str(i).zfill(4)
    init_fn = f"initial{ID}.xyz"
    iso_fn = f"ISOMERS{ID}"
    # os.system(f"cp {zstruct_path}/{init_fn} {ssm_path}/{init_fn}")
    # os.system(f"cp {zstruct_path}/{iso_fn} {ssm_path}/{iso_fn}")
